...
import {AxisOptions, IObj, LWChartAxisStyle} from './utils/interfaces';
import {formatPadding, isInBrowser} from './utils';
import {fillRoundRect} from './utils/tooltip';

export default abstract class Axis<T extends AxisOptions> extends Chart<T> {
  ...
  protected mergeOptions (options?: T) {
    ...
    this.options.axisStyle = Object.assign({}, defaultAxisStyle, this.options.axisStyle, options?.axisStyle);
    // 调用父类合并参数方法，合并公共参数
